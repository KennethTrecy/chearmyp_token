...
mod t {
	...
	#[test]
	fn can_push() {
		let token = Token::Simplex(b"abcde");
		let expected_token = Token::Simplex(b"abcde");
	...
	fn can_pop_on_filled_queue() {
		let token = Token::Simplex(b"fghijkl");
		let expected_token = Some(Token::Simplex(b"fghijkl"));
		let mut queue = TokenQueue::new();
		queue.push(token);
